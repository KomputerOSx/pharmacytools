// "use client";
//
// import React from "react";
// import Link from "next/link";
// import "./AppCards.css";
// import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
// import { IconDefinition } from "@fortawesome/fontawesome-svg-core";
// import {
//     faPhone,
//     faCalculator,
//     faSyringe,
//     faInfoCircle,
//     faClipboardList,
//     faChartLine,
//     faFlask,
//     faBook,
//     faTags,
// } from "@fortawesome/free-solid-svg-icons";
//
// // Map of icon names to actual icons for easy reference
// const iconMap: { [key: string]: IconDefinition } = {
//     phone: faPhone,
//     calculator: faCalculator,
//     syringe: faSyringe,
//     info: faInfoCircle,
//     clipboard: faClipboardList,
//     chart: faChartLine,
//     flask: faFlask,
//     book: faBook,
// };
//
// // App data structure
// interface AppData {
//     title: string;
//     description: string;
//     route: string;
//     icon: string;
//     tags: string[];
//     version: string;
//     author: string;
//     color?: string;
// }
//
// // Component Props
//
// function AppCards() {
//     // Manually defined app data
//     const apps: AppData[] = [
//         {
//             title: "Phone Book",
//             description: "Easily look up the contacts from pharmacy department",
//             route: "/webApps/contacts",
//             icon: "phone",
//             tags: ["pharmacy", "phone", "contacts", "bleeps"],
//             version: "1.0",
//             author: "Ramyar Abdullah",
//             color: "info",
//         },
//         {
//             title: "Taper Calculator",
//             description: "Generate Taper Labels that are not possible in V6",
//             route: "/webApps/taperCalculator",
//             icon: "calculator",
//             tags: ["sei", "eye drops", "oral steroids"],
//             version: "2.0",
//             author: "Ramyar Abdullah",
//             color: "success",
//         },
//         {
//             title: "Creatinine Clearance Calculator",
//             description:
//                 "Use Cockcroft-Gault CrCl Formula to calculate renal function",
//             route: "/webApps/crclCalculator",
//             icon: "info",
//             tags: ["calculator", "creatinine", "crcl"],
//             version: "1.0",
//             author: "Ramyar Abdullah",
//             color: "warning",
//         },
//         {
//             title: "Palliative Label Generator",
//             description: "Generate Palliative Care Labels & speed up your TTOs",
//             route: "/webApps/palliativeLabelGenerator",
//             icon: "syringe",
//             tags: ["palliative", "tto", "eol"],
//             version: "1.0",
//             author: "Ramyar Abdullah",
//             color: "danger",
//         },
//     ];
//
//     // Get the corresponding FontAwesome icon or default to info
//     const getIcon = (iconName: string): IconDefinition => {
//         return iconMap[iconName] || faInfoCircle;
//     };
//
//     return (
//         <div className="app-cards">
//             <div className="container">
//                 <div className="columns is-multiline">
//                     {apps.map((app, index) => (
//                         <div className="column is-3" key={index}>
//                             <Link href={app.route} className="app-link">
//                                 <div
//                                     className={`box app-card has-background-${app.color || "white"}-light`}
//                                 >
//                                     <div
//                                         className={`app-icon has-text-${app.color || "info"}`}
//                                     >
//                                         <FontAwesomeIcon
//                                             icon={getIcon(app.icon)}
//                                             size="2x"
//                                         />
//                                     </div>
//                                     <h3 className="title is-4 mt-3">
//                                         {app.title}
//                                     </h3>
//                                     <p className="app-description mb-4">
//                                         {app.description}
//                                     </p>
//                                     <div className="app-tags mb-2">
//                                         <FontAwesomeIcon
//                                             icon={faTags}
//                                             className="mr-2"
//                                             size="sm"
//                                         />
//                                         <small>{app.tags.join(", ")}</small>
//                                     </div>
//                                     <div className="app-meta">
//                                         <small>
//                                             v{app.version} · By {app.author}
//                                         </small>
//                                     </div>
//                                 </div>
//                             </Link>
//                         </div>
//                     ))}
//                 </div>
//             </div>
//         </div>
//     );
// }
//
// export default AppCards;
"use client";

import React, { useState, useEffect } from "react";
import Link from "next/link";
import "./AppCards.css";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { IconDefinition } from "@fortawesome/fontawesome-svg-core";
import {
    faPhone,
    faCalculator,
    faSyringe,
    faInfoCircle,
    faClipboardList,
    faChartLine,
    faFlask,
    faBook,
    faTags,
    faSpinner,
    faExclamationTriangle,
} from "@fortawesome/free-solid-svg-icons";

// Map of icon names to actual icons for easy reference
const iconMap: { [key: string]: IconDefinition } = {
    phone: faPhone,
    calculator: faCalculator,
    syringe: faSyringe,
    info: faInfoCircle,
    clipboard: faClipboardList,
    chart: faChartLine,
    flask: faFlask,
    book: faBook,
};

// App data structure from the API
interface WebAppData {
    name: string;
    description: string;
    version: string;
    author: string;
    tags: string[];
    icon: string;
    color?: string;
}

interface WebApp {
    id: string;
    path: string;
    data: WebAppData;
}

// Color assignment based on app index
const colors = ["info", "success", "warning", "danger", "primary", "link"];

function AppCards() {
    const [apps, setApps] = useState<WebApp[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    // Removed filter state

    // Fetch apps from the API
    useEffect(() => {
        async function fetchApps() {
            try {
                setIsLoading(true);
                const response = await fetch("/api/webApps");

                if (!response.ok) {
                    throw new Error("Failed to fetch apps");
                }

                const data = await response.json();
                setApps(data);
            } catch (err) {
                setError(
                    err instanceof Error ? err.message : "An error occurred",
                );
            } finally {
                setIsLoading(false);
            }
        }

        fetchApps();
    }, []);

    // No filtering, use all apps
    const filteredApps = apps;

    // Get the corresponding FontAwesome icon or default to info
    const getIcon = (iconName: string): IconDefinition => {
        return iconMap[iconName] || faInfoCircle;
    };

    // Loading state
    if (isLoading) {
        return (
            <div className="app-cards has-text-centered my-6">
                <FontAwesomeIcon
                    icon={faSpinner}
                    spin
                    size="3x"
                    className="has-text-info"
                />
                <p className="mt-3">Loading applications...</p>
            </div>
        );
    }

    // Error state
    if (error) {
        return (
            <div className="app-cards has-text-centered my-6">
                <div className="notification is-danger is-light">
                    <FontAwesomeIcon
                        icon={faExclamationTriangle}
                        className="mr-2"
                    />
                    Error: {error}
                </div>
            </div>
        );
    }

    return (
        <div className="app-cards">
            <div className="container">
                {/* Tag filter section removed */}

                <div className="columns is-multiline">
                    {filteredApps.map((app, index) => (
                        <div className="column is-3" key={app.id}>
                            <Link href={app.path} className="app-link">
                                <div
                                    className={`box app-card has-background-${app.data.color || colors[index % colors.length]}-light`}
                                >
                                    <div
                                        className={`app-icon has-text-${app.data.color || colors[index % colors.length]}`}
                                    >
                                        <FontAwesomeIcon
                                            icon={getIcon(app.data.icon)}
                                            size="2x"
                                        />
                                    </div>
                                    <h3 className="title is-4 mt-3">
                                        {app.data.name}
                                    </h3>
                                    <p className="app-description mb-4">
                                        {app.data.description}
                                    </p>
                                    <div className="app-tags mb-2">
                                        <FontAwesomeIcon
                                            icon={faTags}
                                            className="mr-2"
                                            size="sm"
                                        />
                                        <small>
                                            {app.data.tags.join(", ")}
                                        </small>
                                    </div>
                                    <div className="app-meta">
                                        <small>
                                            v{app.data.version} · By{" "}
                                            {app.data.author}
                                        </small>
                                    </div>
                                </div>
                            </Link>
                        </div>
                    ))}
                </div>

                {/* No results message */}
                {filteredApps.length === 0 && (
                    <div className="has-text-centered my-6">
                        <p className="is-size-5 has-text-grey">
                            No applications found with the selected filter.
                        </p>
                    </div>
                )}
            </div>
        </div>
    );
}

export default AppCards;
